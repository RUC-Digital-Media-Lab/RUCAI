...
if [[ "$SYNC_DB" == "1" ]]; then
  ...
  ssh "${ssh_opts[@]}" "$UCLOUD_SSH" \
    "${src_pw_prefix}pg_dump -Fc -h '$(escape_sq "$UCLOUD_DB_HOST")' -p '$(escape_sq "$UCLOUD_DB_PORT")' -U '$(escape_sq "$UCLOUD_DB_USER")' '$(escape_sq "$UCLOUD_DB_NAME")'" \
    | bash -lc "${dst_pw_prefix}pg_restore --clean --if-exists -h '$(escape_sq "$SERVER_DB_HOST")' -p '$(escape_sq "$SERVER_DB_PORT")' -U '$(escape_sq "$SERVER_DB_USER")' -d '$(escape_sq "$SERVER_DB_NAME")'"
else
